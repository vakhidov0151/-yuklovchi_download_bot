...
async def handle_document(message: types.Message):
    ...
    try:
        ...
        if file_name.lower().endswith('.pdf'):
            short_id = str(uuid.uuid4())[:8]
            url_cache[short_id] = {'file_id': doc.file_id, 'orig_name': file_name}
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
...
async def process_ai_summary(callback_query: types.CallbackQuery):
    ...
    cached = url_cache.get(short_id)
    if not cached or 'file_id' not in cached:
        await callback_query.answer("❌ Kesh tozalandi, faylni boshqatdan tashlang.", show_alert=True)
        return
...
async def process_ai_summary_url(callback_query: types.CallbackQuery):
    ...
    if not cached:
        await callback_query.answer("❌ Kesh tozalandi, linkni qayta yuboring.", show_alert=True)
        return
...
async def process_download(callback_query: types.CallbackQuery):
    ...
    action = data[1]
    video_id = data[2]
    cache_data = url_cache.get(video_id)
    ...
    try:
        if action == "vid":
            ...
            video = FSInputFile(file_path, filename=f"{title}.mp4")
            msg = await callback_query.message.answer_video(video=video, caption=f"🎬 <b>{title}</b>\n💿 {quality}p")
            ...
            msg = await callback_query.message.answer_audio(audio=audio, caption=f"🎵 <b>{title}</b>")
